...
# Modif. : Guillaume MICHON 05/10/2009 (v 1.1 - Utilisation de l'uid du compte appelant pour le passer au script import_su
#                                               + refus d'exécution si le compte lanceur n'est pas $COMPTE_TRANSFERT)
# Modif. : Guillaume MICHON 19/02/2010 (v 1.2 - Passage en itération 2)
#               * Suppression de NFS et de l'utilisation du script import
...
$COMPTE_MTBI = "mtbi";
$CHEMIN_MTBI = "/usr/bin/import_mtbi";
...
$FICLOG = "import_$DATE.log";
$FICLOG_MASQUE = "import_*.log";
$ERREUR_WARNING = 1;
$ERREUR_ARG = 2;
$ERREUR_FATALE = 3;
$NUM = -1;
...
# Retourne : Rien
sub usage {
    print STDERR "Service de transfert (import). Paramètres : <chemin/masque> [<retention en jours> <nb de versions à conserver>]\n";
    print STDERR "L'argument <chemin> peut être un chemin complet décrivant le fichier dans son répertoire cible, ou juste un nom de fichier.\n";
    print STDERR "S'il s'agit d'un simple nom de fichier, il sera transféré vers $DONNEES/import\n";
...
# Fichier et répertoire destinataire
@liste_rep = split(/\//, $masque_fichier);
if (scalar(@liste_rep) == 1) {
    # Le fichier fourni n'a pas de chemin, on part donc du principe qu'il est à importer dans $DONNEES/import
    $rep_donnees = "$DONNEES/import";
}
else {
    # Le fichier est fourni avec son chemin, on le déposera donc à l'endroit indiqué
...
$parametres_mtbi = "-s '$hostname' -r '$rep_donnees' -f '$masque_fichier' -u '$compte'";
