...
version = "transf.py v1.7 - python "+__PYTHON_VERSION+" - "+__SYSTEM
...
def f_param_lg_commande(char_os):
    ...
    try:
        opts, args = getopt.getopt(arguments, "hB:i:o:f:IO",
            ["help","bouchon=","save=","keeptimestamp","force=","no_tmp","no82safe",
            "complete","retention=","version=","onlyone","delta=","debug","cr3"])
...
def f_listdirectory(path, filtre, char_os):
    ...
    if ((vg_complete == True) or (vg_delta <> None) or (vg_onlyone == True)) and (vg_82safe == True) :
        p_debug("option complete, delta, onlyone avec vg_82safe actif")
        chemin_full = path+char_os+"??????????????.????????.??"
        l = glob.glob(chemin_full)
        extension_filtre = True
    elif ((vg_complete == True) or (vg_delta <> None) or (vg_onlyone == True)) and (vg_82safe == False) :
        p_debug("option complete, delta, onlyone avec vg_82safe inactif")
        chemin_full = path+char_os+"??????????????.*"
        l = glob.glob(chemin_full)
        extension_filtre = True
    elif vg_82safe == True :
    ...
    if extension_filtre == True :
        chemin_filtre = path+char_os+"??????????????."+filtre
    else :
        chemin_filtre = path+char_os+filtre
    l_filtree = glob.glob(chemin_filtre)
    p_debug(str(chemin_filtre))
    p_debug(str(l_filtree))
    for i in l:
        if os.path.isfile(i):
            if i in l_filtree :
...
def f_nom_fichier_sans_timestamp(nom):
    # retourne le nom de fichier sans le timestamp avant
    return nom[len(nom)-11:len(nom)]
